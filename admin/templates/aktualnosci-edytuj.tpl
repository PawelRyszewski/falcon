	<div class="container-fluid">
	    <div class="row mb-4">
	        <div class="col-12 col-lg">
	            <h1 class="main-title"><span>Edytuj artykuł</span></h1>
	            <h5>{$page.title}</h5>
	        </div>
	                        ...
	                        <form class="p-3" name="edit-page-form" id="edit-page-form" method="post" enctype="multipart/form-data">
	                            ...
	                            <div class="collapse mb-3" id="ustawieniaSeo">
	                                <div class="card card-body">
	                                    ...
	                                    <div class="form-group">
	                                        ...
	                                        <input type="text" class="form-control" name="url" placeholder="nazwa-twojej-aktualnosci" id="url" value="{$page.url}" onkeydown="timeOutCheckUrl()">
	                            ...
	                            <div class="form-group row">
									...
									<div class="col-4">
										...
										<label class="mt-2">Lub wybierz z galerii</label>
	                                ...
	                                {if $page.img|count_characters}
    	                                <div class="col-4">
    	                                    <p>Aktualne zdjęcie</p>
    	                                    <img src="{$page.img_thumb_path}" />
    	                                </div>
    
    	                                <div class="col-4">
    	                                    <label>Tytuł zdjęcia</label>
    	                                    <input type="text" class="form-control" name="img_title_old" value="{$page.img_title}">
    	                                </div>
	                                {/if}
	                            </div>
                                    ...
                                    <div class="form-group">
                                        <label>Tytuł</label>
                                        <input type="text" class="form-control" name="title" id="title" value="{$page.title}" onblur="javascript: validateTitle(this, this.value);" onkeypress="javascript: validateTitle(this, this.value);">
                                    ...
                                    <div class="form-group">
	                                    ...
	                                    {foreach from=$categories item=item}
    	                                    <option value="{$item.id}" {if $page.category==$item.id}selected{/if}>{$item.name}</option>
	                                    {/foreach}
	                                </select>
	                            </div>
	                            <div class="form-group">
	                                <label>Krótki opis</label>
	                                <textarea class="form-control" name="content_short">{$page.content_short}</textarea>
	                            </div>
	                            ...
	                            <button hidden type="submit" name="edit_news" onClick="javascript: submitForm(event);" class="btn btn-success" id="edit_news">Edytuj podstronę</button>
	                            {if $page.is_published == 0}
    	                            <button type="button" class="btn btn-primary" onClick="validUrlAndSubmitSave()">Zapisz</button>
    	                            <button hidden type="submit" name="edit_news_save" onClick="javascript: submitForm(event);" class="btn btn-success" id="edit_news_save">Zapisz</button>
	                            {/if}
	                        </form>
	...
	<input type="hidden" value="{$id_page}" id="id_page" />
	{literal}
    	<script>
    	    var myVar;
    	    ...
    	    function checkImgName() {
    	        ...
    	        if (elem) {
    	            ...
    	            elem.val(removeDiacritics(imgName, true))
    	        }
    	    }
