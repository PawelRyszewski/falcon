...
if (!defined('LANG_PATH')) {
    define('LANG_PATH', dirname(__DIR__, 3) . '/langs');
}
...
function getLangId(?string $lang = null): int {
    static $map = [];
    ...
    try {
        $db = DatabaseManager::getConnection();
        $stmt = $db->prepare('SELECT id FROM pages_langs WHERE name = ? LIMIT 1');
        $stmt->execute([$lang]);
        $map[$lang] = (int)($stmt->fetchColumn() ?: 1);
    } catch (PDOException $e) {
        $map[$lang] = 1;
    }
    return $map[$lang];
